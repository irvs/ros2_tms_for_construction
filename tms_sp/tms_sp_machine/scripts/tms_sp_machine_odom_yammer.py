...
class TmsSpMachineOdom(Node):
    ...
    def send_odom_to_db_writer(self, msg: Odometry) -> None:
        ...
        """
        crs_geo = CRS.from_epsg(4612)
        crs_plane = CRS.from_epsg(2444)
        transformer = Transformer.from_crs(crs_geo, crs_plane, always_xy=True)
        x, y = transformer.transform(msg.pose.pose.position.y, msg.pose.pose.position.x)
