...
class TmsSsTerrainStaticDem(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("fill_nan_type", "none")  # 'none' or 'avg' or 'median'
        ...
        self.fill_nan_type: str = (
            self.get_parameter("fill_nan_type").get_parameter_value().string_value
        )
    ...
    def convert_pointcloud2_to_dem(self) -> np.ndarray:
        ...
        points = []
        for x, y, z, _, _, _ in list(
            point_cloud2.read_points(self.msg, skip_nans=True, field_names=field_names)
        ):
            points.append([x, y, z])
