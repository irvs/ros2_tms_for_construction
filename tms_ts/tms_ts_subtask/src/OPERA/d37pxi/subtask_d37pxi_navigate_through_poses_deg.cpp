...
#include <vector>
#include "tms_ts_subtask/OPERA/d37pxi/subtask_d37pxi_navigate_through_poses_deg.hpp"
// #include <glog/logging.h>

using std::placeholders::_1;
using std::placeholders::_2;

SubtaskD37pxiNavigateThroughPosesDeg::SubtaskD37pxiNavigateThroughPosesDeg() : SubtaskNodeBase("st_d37pxi_navigate_through_poses_deg_node")
{
    this->action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
        this, "st_d37pxi_navigate_through_poses_deg",
        std::bind(&SubtaskD37pxiNavigateThroughPosesDeg::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&SubtaskD37pxiNavigateThroughPosesDeg::handle_cancel, this, std::placeholders::_1),
        std::bind(&SubtaskD37pxiNavigateThroughPosesDeg::handle_accepted, this, std::placeholders::_1));

    
    action_client_ = rclcpp_action::create_client<NavigateThroughPoses>(this, "navigate_through_poses");
}

rclcpp_action::GoalResponse SubtaskD37pxiNavigateThroughPosesDeg::handle_goal(
...
rclcpp_action::CancelResponse SubtaskD37pxiNavigateThroughPosesDeg::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void SubtaskD37pxiNavigateThroughPosesDeg::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{ std::bind(&SubtaskD37pxiNavigateThroughPosesDeg::execute, this, _1), goal_handle }.detach();
}

void SubtaskD37pxiNavigateThroughPosesDeg::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
    RCLCPP_INFO(this->get_logger(), "subtask(st_d37pxi_navigate_through_poses) is executing...");
...
void SubtaskD37pxiNavigateThroughPosesDeg::goal_response_callback(const GoalHandleD37pxiNavigateThroughPoses::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void SubtaskD37pxiNavigateThroughPosesDeg::feedback_callback(
    const GoalHandleD37pxiNavigateThroughPoses::SharedPtr,
    const std::shared_ptr<const GoalHandleD37pxiNavigateThroughPoses::Feedback> feedback)
{
...
//result
void SubtaskD37pxiNavigateThroughPosesDeg::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
                                             const GoalHandleD37pxiNavigateThroughPoses::WrappedResult& result)
{
  if (!goal_handle->is_active())
  {
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<SubtaskD37pxiNavigateThroughPosesDeg>());
    rclcpp::shutdown();
    return 0;
}
