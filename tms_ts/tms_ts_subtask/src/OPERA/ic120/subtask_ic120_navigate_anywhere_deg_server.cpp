...
#include "tms_ts_subtask/OPERA/ic120/subtask_ic120_navigate_anywhere_deg.hpp"
...
rclcpp_action::GoalResponse SubtaskIc120NavigateAnywhereDeg::handle_goal(
...
rclcpp_action::CancelResponse SubtaskIc120NavigateAnywhereDeg::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void SubtaskIc120NavigateAnywhereDeg::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{ std::bind(&SubtaskIc120NavigateAnywhereDeg::execute, this, _1), goal_handle }.detach();
}

void SubtaskIc120NavigateAnywhereDeg::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
    RCLCPP_INFO(this->get_logger(), "subtask(st_ic120_navigate_anywhere_node) is executing...");
...
void SubtaskIc120NavigateAnywhereDeg::goal_response_callback(const GoalHandleIc120NavigateAnywhere::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void SubtaskIc120NavigateAnywhereDeg::feedback_callback(
    const GoalHandleIc120NavigateAnywhere::SharedPtr,
    const std::shared_ptr<const GoalHandleIc120NavigateAnywhere::Feedback> feedback)
{
...
//result
void SubtaskIc120NavigateAnywhereDeg::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<SubtaskIc120NavigateAnywhereDeg>());
    rclcpp::shutdown();
    return 0;
}
