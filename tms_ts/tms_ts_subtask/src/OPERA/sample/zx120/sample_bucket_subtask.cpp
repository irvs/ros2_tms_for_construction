...
#include "tms_ts_subtask/OPERA/sample/zx120/sample_bucket_subtask.hpp"
#include <glog/logging.h>

using namespace std::chrono_literals;

SubtaskSampleZx120Bucket::SubtaskSampleZx120Bucket() : SubtaskNodeBase("st_zx120_sample_bucket_node")
{
  publisher_ = this->create_publisher<std_msgs::msg::Float64>("/zx120/bucket/cmd", 10);
  this->action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
      this, "st_zx120_sample_bucket",
...
rclcpp_action::GoalResponse SubtaskSampleZx120Bucket::handle_goal(
...
rclcpp_action::CancelResponse SubtaskSampleZx120Bucket::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
  RCLCPP_INFO(this->get_logger(), "Received request to cancel subtask node");
  return rclcpp_action::CancelResponse::ACCEPT;
}

void SubtaskSampleZx120Bucket::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
  using namespace std::placeholders;
  std::thread{ std::bind(&SubtaskSampleZx120Bucket::execute, this, _1), goal_handle }.detach();
}

void SubtaskSampleZx120Bucket::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
  RCLCPP_INFO(this->get_logger(), "subtask is executing...");
  rclcpp::Rate loop_rate(1);
  const auto goal_pos = parameters["zx120_bucket_goal_pos"];
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(std::make_shared<SubtaskSampleZx120Bucket>());
  rclcpp::shutdown();
  return 0;
}
