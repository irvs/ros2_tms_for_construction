...
#include "tms_ts_subtask/crane/zx120/sample_swing_subtask.hpp"
#include <glog/logging.h>

using namespace std::chrono_literals;

SubtaskSampleZx120Swing::SubtaskSampleZx120Swing() : SubtaskNodeBase("subtask_sample_zx120_swing")
{
  publisher_ = this->create_publisher<std_msgs::msg::Float64>("/zx120/swing/cmd", 10);
  this->action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
      this,
      "sample_zx120_swing_subtask",
...
rclcpp_action::GoalResponse SubtaskSampleZx120Swing::handle_goal(const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const tms_msg_ts::action::LeafNodeBase::Goal> goal)
{
    parameters = GetParamFromDB(goal->model_name, goal->component_name);
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse SubtaskSampleZx120Swing::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void SubtaskSampleZx120Swing::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{std::bind(&SubtaskSampleZx120Swing::execute, this, _1), goal_handle}.detach();
}

void SubtaskSampleZx120Swing::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
