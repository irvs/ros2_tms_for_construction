...
#include "tms_ts_subtask/crane/zx200/sample_bucket_subtask.hpp"
...
rclcpp_action::GoalResponse SubtaskSampleZx200Bucket::handle_goal(const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const tms_msg_ts::action::LeafNodeBase::Goal> goal)
{
    ...
    RCLCPP_INFO(this->get_logger(), "param = %d", parameters["zx200_bucket_goal_pos"]);
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

rclcpp_action::CancelResponse SubtaskSampleZx200Bucket::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void SubtaskSampleZx200Bucket::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{std::bind(&SubtaskSampleZx200Bucket::execute, this, _1), goal_handle}.detach();
}

void SubtaskSampleZx200Bucket::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
    RCLCPP_INFO(this->get_logger(), "subtask is executing...");
    rclcpp::Rate loop_rate(1);
    const auto goal_pos = parameters["zx200_bucket_goal_pos"];
    ...
    while(deg >= goal_pos){
        ...
        RCLCPP_INFO_STREAM(this->get_logger(), "Publishing bucket position: " << deg << " [deg]");
        sleep(1); 
    }
...
int main(int argc, char *argv[])
{
    ...
    rclcpp::spin(std::make_shared<SubtaskSampleZx200Bucket>());
    rclcpp::shutdown();
    return 0;
}
