...
rclcpp_action::CancelResponse SubtaskCrawlerDumpReleaseSoil::handle_cancel(
...
void SubtaskCrawlerDumpReleaseSoil::result_callback(
  const std::shared_ptr<GoalHandle> server_goal_handle,
  const rclcpp_action::Client<TmsRpCrawlerDumpDumpAngle>::WrappedResult & result)
{
  ...
  switch (result.code) {
    ...
    case rclcpp_action::ResultCode::ABORTED:
      ...
      RCLCPP_INFO(this->get_logger(), "Subtask execution aborted");
      break;

    case rclcpp_action::ResultCode::CANCELED:
      result_to_leaf->result = false;
      server_goal_handle->canceled(result_to_leaf);
      RCLCPP_INFO(this->get_logger(), "Subtask execution canceled");
      break;
