...
#include "tms_ts_subtask/CrawlerDump/subtask_crawlerdump_follow_path.hpp"
// #include <glog/logging.h>

using std::placeholders::_1;
using std::placeholders::_2;

SubtaskCrawlerDumpFollowPath::SubtaskCrawlerDumpFollowPath() : SubtaskNodeBase("subtask_crawlerdump_follow_path_node")
{
    this->action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
        this, "subtask_crawlerdump_follow_path",
        std::bind(&SubtaskCrawlerDumpFollowPath::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&SubtaskCrawlerDumpFollowPath::handle_cancel, this, std::placeholders::_1),
        std::bind(&SubtaskCrawlerDumpFollowPath::handle_accepted, this, std::placeholders::_1));

    action_client_ = rclcpp_action::create_client<FollowPath>(this, "follow_path");
}

rclcpp_action::GoalResponse SubtaskCrawlerDumpFollowPath::handle_goal(
...
rclcpp_action::CancelResponse SubtaskCrawlerDumpFollowPath::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void SubtaskCrawlerDumpFollowPath::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{ std::bind(&SubtaskCrawlerDumpFollowPath::execute, this, _1), goal_handle }.detach();
}

void SubtaskCrawlerDumpFollowPath::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
    RCLCPP_INFO(this->get_logger(), "subtask(subtask_crawlerdump_follow_path_node) is executing...");
...
void SubtaskCrawlerDumpFollowPath::goal_response_callback(const GoalHandleFollowPath::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void SubtaskCrawlerDumpFollowPath::feedback_callback(
...
void SubtaskCrawlerDumpFollowPath::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<SubtaskCrawlerDumpFollowPath>());
    rclcpp::shutdown();
    return 0;
}
