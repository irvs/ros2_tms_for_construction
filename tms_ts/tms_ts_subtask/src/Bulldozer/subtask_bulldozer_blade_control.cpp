#include "tms_ts_subtask/Bulldozer/subtask_bulldozer_blade_control.hpp"
...
bool SubtaskBulldozerBladeControl::get_required_(const std::string & key, size_t idx, double & out) const
{
...
rclcpp_action::GoalResponse SubtaskBulldozerBladeControl::handle_goal(
...
rclcpp_action::CancelResponse SubtaskBulldozerBladeControl::handle_cancel(
...
void SubtaskBulldozerBladeControl::handle_accepted(const std::shared_ptr<ServerGoalHandle> goal_handle)
{
  std::thread{std::bind(&SubtaskBulldozerBladeControl::execute, this, _1), goal_handle}.detach();
}
...
void SubtaskBulldozerBladeControl::goal_response_callback(const typename ClientGoalHandle::SharedPtr & goal_handle)
{
  if (!goal_handle) {
...
void SubtaskBulldozerBladeControl::result_callback(
  const std::shared_ptr<ServerGoalHandle> server_goal_handle,
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<SubtaskBulldozerBladeControl>());
  rclcpp::shutdown();
  return 0;
}
