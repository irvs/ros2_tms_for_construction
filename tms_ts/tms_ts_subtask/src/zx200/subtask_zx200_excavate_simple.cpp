...
#include "tms_ts_subtask/zx200/subtask_zx200_excavate_simple.hpp"
...
std::map<std::string, double> SubtaskZx200ExcavateSimple::GetParamFromDB(std::string model_name,
...
{
  param_from_db_ = GetParamFromDB(goal->model_name, goal->record_name);
...
void SubtaskZx200ExcavateSimple::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
  using namespace std::placeholders;
  std::thread{ std::bind(&SubtaskZx200ExcavateSimple::execute, this, _1), goal_handle }.detach();
}

void SubtaskZx200ExcavateSimple::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void SubtaskZx200ExcavateSimple::feedback_callback(
    const GoalHandleZx200ExcavateSimple::SharedPtr,
    const std::shared_ptr<const GoalHandleZx200ExcavateSimple::Feedback> feedback)
{
  // TODO: Fix to feedback to leaf node
  RCLCPP_INFO(this->get_logger(), "Feedback received: %s", feedback->state.c_str());
}

void SubtaskZx200ExcavateSimple::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(std::make_shared<SubtaskZx200ExcavateSimple>());
  rclcpp::shutdown();
  return 0;
}
