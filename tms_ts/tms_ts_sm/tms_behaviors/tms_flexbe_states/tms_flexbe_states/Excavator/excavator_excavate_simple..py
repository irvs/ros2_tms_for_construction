...
class ExcavatorExcavateSimple(EventState):
    '''
    Excavate the defined position.
    ...
    def __init__(self, model_name, record_name):
        super().__init__(outcomes=['received', 'aborted', 'no_connection', 'data_error'],
                         output_keys=['data'])
        self._task_id = 13 # Sample。DBに正規・非正規Subtask Nodes用のコレクションの2種を用意し、正規のコレクションにprimitivesを格納。タスクIDをそれに合わせて修正
