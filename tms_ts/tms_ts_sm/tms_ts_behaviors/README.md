# FlexBE States and Behaviors for tms_ts
...
This raw repository has several folders and files with the generic name `tms_ts`.
...
and change the relevant `tms_ts` text to `my_new_project_name` as needed.
...
## Example States in `tms_ts_flexbe_states`
...
## Example Behaviors in `tms_ts_flexbe_behaviors`
...
[`copy_behavior`](tms_ts_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run tms_ts_flexbe_behavior copy_behavior` to see the usage guide. 
