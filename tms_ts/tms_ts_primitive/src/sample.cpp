...
LeafNodeBase::LeafNodeBase(const std::string& name, const NodeConfiguration& config) : CoroActionNode(name, config)
{
  ...
  primitive_name_ = primitive_name.value();
...
// action client„ÇíÊñ∞„Åü„Å´‰ΩúÊ?ê„Åô„ÇãÈñ¢Êï∞
...
// action client„ÅÆgoal,feedback,result„Å´ÂØæÂøú„Åô„ÇãcallbackÈñ¢Êï∞„Çí„Åì„ÅÆÈñ¢Êï∞ÂÜ?„ÅßÂÆöÁæ©
...
bool LeafNodeBase::is_future_goal_handle_complete(std::chrono::milliseconds& elapsed)
{
  ...
  {
    future_goal_handle_.reset(); // èCê≥åÛï‚1
    return false;
  }
  ...
  {
    future_goal_handle_.reset(); // èCê≥åÛï‚2
...
void LeafNodeBase::halt()
{
    if (should_cancel_goal())
    {
        ...
        while (true) 
        {
            ...
            {
                RCLCPP_INFO(node_->get_logger(), "Goal for %s successfully canceled.", primitive_name_.c_str());
    ...
    {
        RCLCPP_WARN(node_->get_logger(), "No active goal to cancel for %s", primitive_name_.c_str());
    }
  ...
  try
  {
    if (future_goal_handle_)
    {
      ...
      if (!is_future_goal_handle_complete(elapsed))
      {
        ...
        RCLCPP_WARN(node_->get_logger(), "Timed out while waiting for primitive to acknowledge goal request for %s",
    ...
    if (rclcpp::ok() && !goal_result_available_)
    {
      ...
      // if (goal_updated_ && (goal_status == action_msgs::msg::GoalStatus::STATUS_EXECUTING || // goal_updateïœêîÇÕfalseÇ≈èâä˙âªÇ≥ÇÍÇƒÇ¢ÇÈÇÃÇ≈ÅAÇ±ÇÃèåèÇÕèÌÇ…false
