...
#include "tms_ts_primitive/Excavator/primitive_excavator_level.hpp"
#include <glog/logging.h>

using namespace std::chrono_literals;

PrimitiveExcavatorLevel::PrimitiveExcavatorLevel() : PrimitiveNodeBase("primitive_excavator_level_node")
{
  ...
  action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
      this, "primitive_excavator_level",
      std::bind(&PrimitiveExcavatorLevel::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&PrimitiveExcavatorLevel::handle_cancel, this, std::placeholders::_1),
      std::bind(&PrimitiveExcavatorLevel::handle_accepted, this, std::placeholders::_1),
      options_server);

  action_client_ = rclcpp_action::create_client<ExcavatorChangePose>(this, "tms_rp_level",nullptr ,options_client);
...
rclcpp_action::GoalResponse PrimitiveExcavatorLevel::handle_goal(
    const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const tms_msg_ts::action::LeafNodeBase::Goal> goal)
{
  ...
  param_from_db_ = CustomGetParamFromDB<std::pair<std::string, std::string>, double>(goal->model_name, goal->record_name);
...
rclcpp_action::CancelResponse PrimitiveExcavatorLevel::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void PrimitiveExcavatorLevel::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
  using namespace std::placeholders;
  std::thread{ std::bind(&PrimitiveExcavatorLevel::execute, this, _1), goal_handle }.detach();
}

void PrimitiveExcavatorLevel::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
  ...
  auto goal_msg = ExcavatorChangePose::Goal();
  if (param_from_db_[std::make_pair("trajectory", "")] > 0.5)
  {
    ...
    int point_num = 0;
    for (const auto& pair : param_from_db_) {
      if (pair.first.first == "x") {
        point_num++;
      }
    }
    ...
    for (int i=0; i < point_num; i++){
      ...
      {
        target_pose.positions.push_back(param_from_db_[std::make_pair(joint_name,std::to_string(i))]);
      }
  ...
  else if (param_from_db_[std::make_pair("pose", "")] > 0.5)
  {
    ...
    for (int i=0; i < point_num; i++){
      target_pose.position.x = param_from_db_[std::make_pair("x",std::to_string(i))];
      target_pose.position.y = param_from_db_[std::make_pair("y",std::to_string(i))];
      target_pose.position.z = param_from_db_[std::make_pair("z",std::to_string(i))];
      target_pose.orientation.x = param_from_db_[std::make_pair("qx",std::to_string(i))];
      target_pose.orientation.y = param_from_db_[std::make_pair("qy",std::to_string(i))];
      target_pose.orientation.z = param_from_db_[std::make_pair("qz",std::to_string(i))];
      target_pose.orientation.w = param_from_db_[std::make_pair("qw",std::to_string(i))];
      goal_msg.pose_sequence.push_back(target_pose);
    }

    
  }
  else if (param_from_db_[std::make_pair("position_with_angle", "")] > 0.5)
  {
    RCLCPP_INFO(this->get_logger(), "Get position_with_angle from DB.");
      // xキーの数を数えてpoint_numを動的に計算
    int point_num = 0;
    for (const auto& pair : param_from_db_) {
      if (pair.first.first == "x") {
        point_num++;
    ...
    for (int i=0; i < point_num; i++){
      ...
      target_pose.position.y = param_from_db_[std::make_pair("y",std::to_string(i))];
      target_pose.position.z = param_from_db_[std::make_pair("z",std::to_string(i))];
      target_pose.theta_w = param_from_db_[std::make_pair("theta_w",std::to_string(i))];
      goal_msg.position_with_angle_sequence.push_back(target_pose);
    }
    
    RCLCPP_INFO(this->get_logger(), "DEBUG: position_with_angle_sequence.size() = %zu", point_num);;
  }
  else
  {
  ...
  // Send goal to TMS_RP
  auto send_goal_options = rclcpp_action::Client<ExcavatorChangePose>::SendGoalOptions();
...
void PrimitiveExcavatorLevel::goal_response_callback(const GoalHandleExcavatorChangePose::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void PrimitiveExcavatorLevel::feedback_callback(
...
void PrimitiveExcavatorLevel::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(std::make_shared<PrimitiveExcavatorLevel>());
  rclcpp::shutdown();
  return 0;
}
