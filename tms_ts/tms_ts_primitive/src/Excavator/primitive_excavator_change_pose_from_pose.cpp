...
#include "tms_ts_primitive/Excavator/primitive_excavator_change_pose_from_pose.hpp"
...
PrimitiveExcavatorChangePoseFromPose::PrimitiveExcavatorChangePoseFromPose() : PrimitiveNodeBase("primitive_excavator_change_pose_from_pose_node")
{
  ...
  action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
      this, "primitive_excavator_change_pose_from_pose",
  ...
  action_client_ = rclcpp_action::create_client<ExcavatorChangePoseFromPose>(this, "tms_rp_excavator_change_pose_from_pose",nullptr ,options_client);
...
void PrimitiveExcavatorChangePoseFromPose::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
  ...
  try {
    // 必須パラメータのチェック
    if (!param_from_db_.count("x") || !param_from_db_.count("y") || 
        !param_from_db_.count("z") || !param_from_db_.count("theta_w")) {
      RCLCPP_ERROR(this->get_logger(), "Missing required parameters (x, y, z, theta_w)");
      handle_error("Missing required parameters");
      return;
    }

    // JSON文字列から数値を抽出するヘルパー関数（エラー時は例外を投げる）
    auto parse_double = [this](const std::string& json_str, const std::string& key) -> double {
      ...
      RCLCPP_ERROR(this->get_logger(), "Failed to parse double value for key: %s", key.c_str());
      throw std::runtime_error("Failed to parse double value for key: " + key);
    };

    // position_with_angle_sequenceの構築
    tms_msg_rp::msg::TmsRpExcavatorPositionWithAngle target_pose;
    target_pose.position.x = parse_double(param_from_db_["x"], "x");
    target_pose.position.y = parse_double(param_from_db_["y"], "y");
    target_pose.position.z = parse_double(param_from_db_["z"], "z");
    target_pose.theta_w = parse_double(param_from_db_["theta_w"], "theta_w");
    
    goal_msg.position_with_angle_sequence.push_back(target_pose);
    RCLCPP_INFO(this->get_logger(), "Set target pose: x=%f, y=%f, z=%f, theta_w=%f", 
