...
#include "tms_ts_primitive/Excavator/assist_excavation_pose_to_joint_angles.hpp"
#include <glog/logging.h>
#include <rapidjson/document.h>

using namespace std::chrono_literals;

// Helper function to parse JSON string and extract numeric value
double parseJsonValue(const std::string& json_str, const std::string& key) {
  rapidjson::Document doc;
  doc.Parse(json_str.c_str());
  
  if (doc.HasParseError()) {
    throw std::runtime_error("JSON parse error for key: " + key);
  }
  ...
  throw std::runtime_error("Failed to extract numeric value for key: " + key);
}

AssistExcavationPoseToJointAngles::AssistExcavationPoseToJointAngles() : AssistNodeBase("assist_excavation_pose_to_joint_angles")
{
  ...
  action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
      this, "assist_excavation_pose_to_joint_angles",
      std::bind(&AssistExcavationPoseToJointAngles::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
      std::bind(&AssistExcavationPoseToJointAngles::handle_cancel, this, std::placeholders::_1),
      std::bind(&AssistExcavationPoseToJointAngles::handle_accepted, this, std::placeholders::_1),
      options_server);

  action_client_ = rclcpp_action::create_client<ExcavatorAssist>(this, "tms_rp_excavator_assist_excavation_pose_to_joint_angles", nullptr, options_client);
...
rclcpp_action::GoalResponse AssistExcavationPoseToJointAngles::handle_goal(
...
rclcpp_action::CancelResponse AssistExcavationPoseToJointAngles::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
  RCLCPP_INFO(this->get_logger(), "Received request to cancel Assist node");
...
void AssistExcavationPoseToJointAngles::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
  using namespace std::placeholders;
  std::thread{ std::bind(&AssistExcavationPoseToJointAngles::execute, this, _1), goal_handle }.detach();
}

void AssistExcavationPoseToJointAngles::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
  ...
  auto goal_msg = ExcavatorAssist::Goal();
  RCLCPP_INFO(this->get_logger(), "Get parameters from DB.");
  ...
  // データベースからx, y, z, theta_wを取得
  try {
    // 必須パラメータのチェック
    goal_msg.position_with_angle.position.x = parseJsonValue(param_from_db_["x"], "x");
    goal_msg.position_with_angle.position.y = parseJsonValue(param_from_db_["y"], "y");
    goal_msg.position_with_angle.position.z = parseJsonValue(param_from_db_["z"], "z");
    goal_msg.position_with_angle.theta_w = parseJsonValue(param_from_db_["theta_w"], "theta_w");

    RCLCPP_INFO(this->get_logger(), "Retrieved parameters from DB:");
    RCLCPP_INFO(this->get_logger(), "  x: %f", goal_msg.position_with_angle.position.x);
    RCLCPP_INFO(this->get_logger(), "  y: %f", goal_msg.position_with_angle.position.y);
    RCLCPP_INFO(this->get_logger(), "  z: %f", goal_msg.position_with_angle.position.z);
    RCLCPP_INFO(this->get_logger(), "  theta_w: %f", goal_msg.position_with_angle.theta_w);
  ...
  // Send goal to TMS_RP
  auto send_goal_options = rclcpp_action::Client<ExcavatorAssist>::SendGoalOptions();
...
void AssistExcavationPoseToJointAngles::goal_response_callback(const GoalHandleExcavatorAssist::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void AssistExcavationPoseToJointAngles::feedback_callback(
  GoalHandleExcavatorAssist::SharedPtr,
...
void AssistExcavationPoseToJointAngles::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
                                           const GoalHandleExcavatorAssist::WrappedResult& result)
{
  ...
  switch (result.code)
  {
    case rclcpp_action::ResultCode::SUCCEEDED:
      ...
      RCLCPP_INFO(this->get_logger(), "Assist execution is succeeded");
      break;
    case rclcpp_action::ResultCode::ABORTED:
      result_to_leaf->result = false;
      goal_handle->abort(result_to_leaf);
      RCLCPP_INFO(this->get_logger(), "Assist execution is aborted");
      break;
    case rclcpp_action::ResultCode::CANCELED:
      result_to_leaf->result = false;
      goal_handle->canceled(result_to_leaf);
      RCLCPP_INFO(this->get_logger(), "Assist execution is canceled");
      break;
    default:
      result_to_leaf->result = false;
      goal_handle->abort(result_to_leaf);
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(std::make_shared<AssistExcavationPoseToJointAngles>());
  rclcpp::shutdown();
  return 0;
}
