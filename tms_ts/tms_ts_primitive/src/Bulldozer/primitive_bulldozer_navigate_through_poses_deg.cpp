...
#include <vector>
#include "tms_ts_primitive/Bulldozer/primitive_bulldozer_navigate_through_poses_deg.hpp"
// #include <glog/logging.h>

using std::placeholders::_1;
using std::placeholders::_2;

PrimitiveBulldozerNavigateThroughPosesDeg::PrimitiveBulldozerNavigateThroughPosesDeg() : PrimitiveNodeBase("primitive_bulldozer_navigate_through_poses_deg_node")
{
    this->action_server_ = rclcpp_action::create_server<tms_msg_ts::action::LeafNodeBase>(
        this, "primitive_bulldozer_navigate_through_poses_deg",
        std::bind(&PrimitiveBulldozerNavigateThroughPosesDeg::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
        std::bind(&PrimitiveBulldozerNavigateThroughPosesDeg::handle_cancel, this, std::placeholders::_1),
        std::bind(&PrimitiveBulldozerNavigateThroughPosesDeg::handle_accepted, this, std::placeholders::_1));

    
    action_client_ = rclcpp_action::create_client<NavigateThroughPoses>(this, "tms_rp_navigate_through_poses_deg");
}

rclcpp_action::GoalResponse PrimitiveBulldozerNavigateThroughPosesDeg::handle_goal(
...
rclcpp_action::CancelResponse PrimitiveBulldozerNavigateThroughPosesDeg::handle_cancel(const std::shared_ptr<GoalHandle> goal_handle)
{
...
void PrimitiveBulldozerNavigateThroughPosesDeg::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle)
{
    using namespace std::placeholders;
    std::thread{ std::bind(&PrimitiveBulldozerNavigateThroughPosesDeg::execute, this, _1), goal_handle }.detach();
}

void PrimitiveBulldozerNavigateThroughPosesDeg::execute(const std::shared_ptr<GoalHandle> goal_handle)
{
    RCLCPP_INFO(this->get_logger(), "primitive(primitive_bulldozer_navigate_through_poses) is executing...");
...
void PrimitiveBulldozerNavigateThroughPosesDeg::goal_response_callback(const GoalHandleBulldozerNavigateThroughPoses::SharedPtr& goal_handle)
{
  if (!goal_handle)
  {
...
void PrimitiveBulldozerNavigateThroughPosesDeg::feedback_callback(
    const GoalHandleBulldozerNavigateThroughPoses::SharedPtr,
    const std::shared_ptr<const GoalHandleBulldozerNavigateThroughPoses::Feedback> feedback)
{
...
//result
void PrimitiveBulldozerNavigateThroughPosesDeg::result_callback(const std::shared_ptr<GoalHandle> goal_handle,
                                             const GoalHandleBulldozerNavigateThroughPoses::WrappedResult& result)
{
  if (!goal_handle->is_active())
  {
...
int main(int argc, char* argv[])
{
    ...
    rclcpp::spin(std::make_shared<PrimitiveBulldozerNavigateThroughPosesDeg>());
    rclcpp::shutdown();
    return 0;
}
