...
class GUI_button(Node):
    ...
    def __init__(self):
        super().__init__('button_input_bt_')

        self.declare_parameter('task_id', 3)
        self.task_id = self.get_parameter("task_id").get_parameter_value().integer_value
        self.emergency_signal_publisher = self.create_publisher(Bool, '/emergency_signal2', 10)
        self.publisher_ = self.create_publisher(String, '/task_sequence2', 10)
        root = tk.Tk()
    ...
    def set_parameters(self):
        # 任意�?�パラメータ判別用記号(�?: ${parameter})などをtask_sequenceの中に入れて、この部�?を置き換えるプログラ�?を作�?�す�?
        client = MongoClient(MONGODB_IPADDRESS, MONGODB_PORTNUMBER)
        db = client['rostmsdb']
